...
def main():
    ...
    try:
        nltk.download('punkt_tab', quiet=True) # quiet=True 可以讓日誌乾淨一點
        # 如果你的 NLTK 版本較舊，它可能需要的是舊版的 'punkt'，可以保險起見一併加上：
    ...
    # 初始化你剛升級為 BERT 版的 Extractor
    extractor = ScientificPaperExtractor(top_k=30)
    
    # 執行純本地的向量編碼與 Cosine 相似度計算
